...
class TransientThermalModel:
    def __init__(self, k, d_b, Td, T_inf, L, Tip_grid, Q_bias, sigma, epsilon_2, P_Torr, alpha, rho, c, a_cooler, b_cooler, time_steps, activation='tanh', initializer='glorot_normal'):
        self.k = k
        self.d_b = d_b
        self.Td = Td
        self.T_inf = T_inf
        ...
        self.x_values = np.concatenate((np.linspace(0, 0.98 * self.L, num=50, endpoint=False),
                                        np.linspace(0.98 * self.L, self.L, num=100+1))).reshape(-1, 1).astype(np.float32)
    ...
    def data_sampling(self):
        X_0 = get_X0(self.lb, self.ub, 400)
        X_b_0, X_b_L = get_XB(self.lb, self.ub, 400)
        X_r = get_Xr(self.lb, self.ub, 2000)
    ...
    def compute_loss(self):
        ...
        total_loss = 1e3 * pde_loss + initial_condition + boundary_condition_x0 + 1e4 * boundary_condition_cooler
    ...
    def get_weight_tensor(self):
        weight_tensors = [tf.reshape(v, [-1]) for v in self.trainable_parameters]
        return tf.concat(weight_tensors, axis=0)

    def set_weight_tensor(self, weight_tensor):
        idx = 0
        for v in self.trainable_parameters:
            shape = v.shape
            size = np.prod(shape)
            new_value = tf.reshape(weight_tensor[idx:idx + size], shape)
            v.assign(new_value)
            idx += size
    
    def save_weights(self, file_name="model_weights.npy"):
        weights = self.get_weight_tensor()
        np.save(file_name, weights.numpy())  # 텐서를 Numpy로 변환 후 저장
        print(f"Weights saved to {file_name}.")

    def load_weights(self, file_name="model_weights.npy"):
        if os.path.exists(file_name):
            weights = np.load(file_name, allow_pickle=True)
            weight_tensor = tf.convert_to_tensor(weights, dtype=self.DTYPE)  # Numpy 배열을 텐서로 변환
            self.set_weight_tensor(weight_tensor)
            print(f"Weights loaded from {file_name}.")
        else:
            print(f"File {file_name} not found. Initializing with random weights.")
