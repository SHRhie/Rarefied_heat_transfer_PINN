...
class TransientThermalModel:
    def __init__(self, k, d_b, Td, T_inf, L, Tip_grid, Q_bias, sigma, epsilon_2, P_Torr, initial_point, alpha, rho, c, a_cooler, b_cooler, time_steps, target_time, target_temp, activation='tanh', initializer='glorot_normal'):
        #self.k = k
        self.d_b = d_b
        self.Td = Td
        self.T_inf = T_inf
        ...
        self.k = tf.Variable(initial_point, trainable=True, dtype=tf.float32, name="k")
        ...
        self.t_values = np.linspace(0, self.time_steps, num=1000)
    ...
    def compute_loss(self):
        ...
        cooldown_loss = tf.reduce_mean(tf.square((77 - predicted_temp_at_target)))

        total_loss = 1e3 * pde_loss + initial_condition + boundary_condition_x0 + 1e3 * boundary_condition_cooler + cooldown_loss
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0:
                ...
                #print(f'Solving parameter: {self.para[0].numpy()}')
...
for material in materials:
    print(f"\nTraining for material: {material['name']}")
    # 모델 초기화
    model = TransientThermalModel(
        k=material["k"], d_b=0.009, Td=77, T_inf=T_inf, L=0.048, 
        Tip_grid=Tip_grid, Q_bias=0, sigma=5.67e-8, epsilon_2=material["epsilon"], 
        P_Torr=1e-4, alpha=1e-6, rho=material["rho"], c=material["c"],
        a_cooler = 0.039, b_cooler = -2,
        time_steps=60, target_time=target_time, target_temp=target_temp, initial_point=initial_point
    )
...
data = {
    "Time (s)": model.t_values,
    "Predicted Temperature (K)": T_pred_at_L
}
'''
# 데이터프레임 생성
...
# CSV 파일로 저장
output_file_path = f'/Users/SHR/Desktop/Temp_results/transient_inverse_results_{target_time}.csv'
